...
namespace Lamentationofrevenge
{
	public class ADVPart : BaseScene
	{
		...
		private Vector2[] _leftCursolPosition =
		{
			new Vector2(280,416),//中央
			new Vector2(280,380),//背景用座標
			new Vector2(280,344),//背景用座標
			new Vector2(280,308),//中央
			new Vector2(280,272),//左
		};
		
		private Vector2[] _rightCursolPosition =
		{
			new Vector2(680,416),//中央
			new Vector2(680,380),//背景用座標
			new Vector2(680,344),//背景用座標
			new Vector2(680,308),//中央
			new Vector2(680,272),//左
		};
		...
		private void AddSelect()
		{
			if(_novel.SelectMessage().Count() == 2)
			{
				...
				ConvertToImage(_selectText.ElementAt(0),_selectPosition[3].X , _selectPosition[3].Y , 750 , 50);
				ConvertToImage(_selectText.ElementAt(1),_selectPosition[1].X , _selectPosition[1].Y , 750 , 50);
			}
			...
			if(_novel.SelectMessage().Count() == 3)
			{
				for(int i = 0 ; i < _selectText.Count();i++)
				{
					...
					ConvertToImage( _selectText.ElementAt(i) , _selectPosition[4 - i * 2].X , _selectPosition[i * 2].Y , 750 , 50);
				}
			}
		}
		...
		{
			if(_novel.IsStill() == false)AddGraphic(_folderPass[0] + _novel.BackGroundGraphicPass(),_graphicPosition[0]);
			else AddGraphic(_folderPass[0] + _novel.StillName(),_graphicPosition[0]);
			
			if(_displayCharaList.Count == 1 && _novel.IsStill() == false)
			{
				AddGraphic(_folderPass[0] + _folderType[3] + _displayCharaList[0],_graphicPosition[1]);
			}	
			if(_displayCharaList.Count == 2 && _novel.IsStill() == false)
			{
			...
			if(_displayCharaList.Count == 3 && _novel.IsStill() == false)
			{
				AddGraphic(_folderPass[0] + _folderType[3] + _displayCharaList[0],_graphicPosition[2]);
				AddGraphic(_folderPass[0] + _folderType[3] + _displayCharaList[1],_graphicPosition[3]);
		...
		public void NameWindowChange()
		{
			...
			if(_novel.Name() == "ドロシア"){AddGraphic(pass + "dorosia.png",_graphicPosition[5]);return;}
		...
		{
			if(_novel.IsClear() == true)
			{
				RemoveScene(Children.Count());
				_displayCharaList.Clear();
		...
		public void GoNext()
		{
			...
			ContorolSelect(_novel.SelectID());
		...
		{
			if(_novel.BackGroundGraphicPass() != null && _novel.IsStill() == false)
			{
			...
			if(_novel.StillName() != null)
			{
				if(_displayMaterialList.Count == 0)_displayMaterialList.Add(_novel.StillName());
				if(_displayMaterialList.First() != _novel.StillName())
				{
					_displayMaterialList[0] = _novel.StillName();
					RemoveScene(Children.Count());
					GoNext();
				}
				return;
		...
		private void ContorolSelect(int selectId)
		{
			if(selectId == 1)
			{
				GoNext();
			...
			{
				_nextScene = "ADVPart";
				_nextText = _selectText.ElementAt(_choiceSelect);
				_bgmPlayer.Dispose();
			}
		}
		...
		{
			return "/Application/data/text/" + _nextText + ".txt";
		}
		...
		private void ButtonContorol ()
		{
			...
			if(Input2.GamePad0.Up.Press && _novel.SelectID() == 2 && _choiceSelect > 0)
			{
				_choiceSelect--;
				...
				{
					AddGraphic("/Application/data/ui/leftcursol.png" , _leftCursolPosition[_choiceSelect * 2 + 1]);
					AddGraphic("/Application/data/ui/rightcursol.png" , _rightCursolPosition[_choiceSelect * 2 + 1]);
				}
				if(_novel.SelectMessageIndex() == 3)
				{
					AddGraphic("/Application/data/ui/leftcursol.png" , _leftCursolPosition[_choiceSelect * 2]);
					AddGraphic("/Application/data/ui/rightcursol.png" , _rightCursolPosition[_choiceSelect * 2 ]);
				}
			}
			if(Input2.GamePad0.Down.Press && _novel.SelectID() == 2 && _choiceSelect < _novel.SelectMessageIndex()-1)
			{
				_choiceSelect++;
				RemoveChild(Children.Last(),true);
				RemoveChild(Children.Last(),true);
